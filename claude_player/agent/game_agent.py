...
class GameAgent:
    ...
    def __init__(self, config: ConfigClass):
        ...
        if hasattr(self.config,
                   'ENABLE_SOUND') and self.config.ENABLE_SOUND and self.config.EMULATION_MODE == "continuous":
        ...
        self.config.NUMBER_PLAYERS = len(self.agents)
    ...
    def get_ai_response(self, agent_index):
        ...
        try:
            ...
            system_prompt = self.agents[agent_index].generate_system_prompt(agent_index)
            ...
            if hasattr(self.game_state, 'runtime_thinking_enabled'):
                action_config["THINKING"] = self.config.MODEL_DEFAULTS.get("THINKING",
                                                                           True) and self.game_state.runtime_thinking_enabled

            # Send request to Claude
            message = self.agents[agent_index].send_request(
                action_config,
                system_prompt,
                self.chat_history[agent_index],
                tools
            )
            ...
            self.chat_history[agent_index].append(assistant_message)
    ...
    def process_tool_results(self, message_content, agent_index, execute_tools=True):
        ...
        for tool_use in tool_use_blocks:
            ...
            if tool_name == "send_inputs" and not execute_tools:
                ...
                logging.info(
                    f"Queued input for later execution: {tool_input['inputs']} (queue size: {len(pending_actions)})")
        ...
        if tool_results:
            ...
            self.chat_history[agent_index].append(tool_results_message)
        ...
        if len(self.chat_history[agent_index]) > self.config.MAX_HISTORY_MESSAGES:
            self.chat_history[agent_index] = self.chat_history[agent_index][-self.config.MAX_HISTORY_MESSAGES:]
    ...
    def run_turn(self, agent_index):
        ...
        try:
            ...
            self.process_tool_results(message_content, agent_index, execute_tools=True)
            ...
            if len(self.chat_history[agent_index]) >= 2 and self.chat_history[agent_index][-1]["role"] == "assistant":
                assistant_content = self.chat_history[agent_index][-1]["content"]
    ...
    def run_continuous(self):
        ...
        print(
            f"Running in continuous mode at 1x speed with analysis every {self.config.CONTINUOUS_ANALYSIS_INTERVAL} seconds")
        ...
        def run_analysis():
            ...
            try:
                ...
                for agent_index in range(len(self.agents)):
                    ...
                    actions = self.process_tool_results(message_content, agent_index, execute_tools=False)
                    ...
                    logging.info(
                        f"Analysis took {last_analysis_duration:.2f} seconds, adaptive interval: {adaptive_interval:.2f}s")
                ...
                with lock:
                    ...
                    if self.error_count > 3:
                        logging.warning(
                            f"Multiple errors ({self.error_count}) detected, increasing delay between analyses")
                ...
                elif len(self.chat_history[agent_index]) >= 2 and self.chat_history[agent_index][-1]["role"] == "assistant":
                    assistant_content = self.chat_history[agent_index][-1]["content"]
                    # Fix: Use proper access method for the content blocks based on their type
                    tool_use_blocks = []
                    for block in assistant_content:
